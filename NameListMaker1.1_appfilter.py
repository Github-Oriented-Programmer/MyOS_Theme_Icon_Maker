import os
with open('appfilter.xml', encoding='utf-8') as name_o:
...
with open('NameList.csv', "w",encoding='utf-8') as name_list:
    for line in contents_by_line:
        if line.find('ComponentInfo')!=-1 :
            AppName=''#appfilter格式无法获取到应用名称
            PkgName=line[line.find('{')+1:line.find('/',line.find('{')+1)]
            StartActivity=line[line.find('/',line.find('{')+1)+1:line.find('}')]
